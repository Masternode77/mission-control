...
export async function GET(_request: NextRequest) {
  try {
    ...
    const taskGraphRows = queryAll<{
      ...
      `
      SELECT task_id, title, status, parent_task_id, owner_role_id, updated_at
      FROM swarm_tasks
    ...
    const task_nodes: TaskGraphNode[] = taskGraphRows.map((t) => ({
      id: `task:${t.task_id}`,
      task_id: t.task_id,
      title: t.title,
      status: t.status,
    ...
    const taskNodeIds = new Set(task_nodes.map((n) => n.task_id));
    const task_edges: TaskGraphEdge[] = task_nodes
      .filter((n) => n.parent_task_id && taskNodeIds.has(String(n.parent_task_id)))
      .map((n) => ({
        id: `task-edge:${n.parent_task_id}->${n.task_id}`,
        source_task_id: String(n.parent_task_id),
        target_task_id: n.task_id,
      }));
